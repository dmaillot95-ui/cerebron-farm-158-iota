import json,pathlib
R=pathlib.Path(__file__).resolve().parent;N="IOTA"
...
assert a["runtime_changed"] is False;assert a["campaign"]["passes"]==10;assert b["m6_rule"]=="M6_NEVER_IN_TRAIN";assert "M6_COLD_BENCHMARK" in m["training_forbidden"]
print(json.dumps({"status":"PASS","name":N,"runtime_changed":False,"m6_guard":True,"passes":10}))
