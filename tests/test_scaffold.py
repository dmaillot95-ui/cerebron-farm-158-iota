...
class T(unittest.TestCase):
 def test_arch(self):
  a=json.loads((ROOT/"architecture"/"IOTA_ARCHITECTURE_V1.json").read_text());self.assertFalse(a["runtime_changed"]);self.assertEqual(a["campaign"]["passes"],10);self.assertEqual(a["role_id"],"ATOMIC_CLAIM_INSPECTOR")
